...
void thread_func(void *arg)
{
    ...
    int end = minim((data->thread_id + 1) * (double)data->N / data->P, (double)data->N);
...
ppm_image *rescale_image(thread_data_t data)
{
    uint8_t sample[3];
    ...
    data.new_image = (ppm_image *)malloc(sizeof(ppm_image));
    ...
    data.new_image->x = RESCALE_X;
    data.new_image->y = RESCALE_Y;

    data.new_image->data = (ppm_pixel *)malloc(data.new_image->x * data.new_image->y * sizeof(ppm_pixel));
    if (!data.new_image)
    {
        fprintf(stderr, "Unable to allocate memory\n");
        exit(1);
    ...
    data.N = data.new_image->x;

    pthread_t threads[data.P];
    thread_data_t datai[data.P];
    int i;
    ...
    for (i = 0; i < data.P; i++)
    {
        ...
        datai[i].N = data.N;
        datai[i].P = data.P;
        datai[i].image = data.image;
        datai[i].new_image = data.new_image;
        pthread_create(&threads[i], NULL, thread_func, &datai[i]);
    }
...
int main(int argc, char *argv[])
{
    ...
    // 1. Rescale the image
    ppm_image *scaled_image = rescale_image(data);
